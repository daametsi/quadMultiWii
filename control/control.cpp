...
void loop ()
{
	...
	{ // 50Hz
		...
		//printf("BOXACC: %d || %d && %d\n",sysInfo.rcOptions[BOXACC], (sysInfo.failsafeCnt > 5*FAILSAVE_DELAY), ACC);
	...
	for(axis=0;axis<3;axis++)
	{
		...
		{ //ACRO MODE or YAW axis
		  if (abs(sysInfo.rcCommand[axis])<350)
			...
			error = (int32_t)sysInfo.rcCommand[axis]<<1 ; //32 bits is needed for calculation: 500*5*10*8 = 200000   16 bits is ok for result if P8>2 (P>0.2)
		  error -= sysInfo.gyroData[axis];

		  PTerm = (sysInfo.rcCommand[axis] * conf.P8[axis])>>6;
		...
		{
			PTerm -=          (sysInfo.gyroData[axis]*sysInfo.dynP8[axis])>>6; //16 bits is needed for calculation   160*200 = 32000         16 bits is ok for result
		}
		else
		{
			PTerm -= ((int32_t)sysInfo.gyroData[axis]*sysInfo.dynP8[axis])>>6; //32 bits is needed for calculation
		}
...
void getEstimatedAttitude(){
  ...
  //printf("GYRO= x: %d, y: %d, z: %d\n", sysInfo.gyroData[ROLL], sysInfo.gyroData[PITCH], sysInfo.gyroData[YAW]);

  // Initialization
  for (axis = 0; axis < 3; axis++) {
    deltaGyroAngle[axis] = sysInfo.gyroData[axis]  * scale;
    sysInfo.accSmooth[axis]  = accLPF[axis]>>ACC_LPF_FACTOR;
    accLPF[axis]      += sysInfo.accADC[axis] - sysInfo.accSmooth[axis];
    // used to calculate later the magnitude of acc vector
    accMag   += sysInfo.accSmooth[axis] * sysInfo.accSmooth[axis];
  ...
  //printf("accMag: %d\n",accMag);
  //printf("GYRO= x: %d, y: %d, z: %d\n", deltaGyroAngle[ROLL], deltaGyroAngle[PITCH], deltaGyroAngle[YAW]);

  rotateV(&EstG.V,deltaGyroAngle);
  //rotateV(&EstG.V,sysInfo.gyroData);

  //printf("sysInfo.accSmooth[PITCH]: %d f.SMALL_ANGLES: %d\n", sysInfo.accSmooth[PITCH], f.SMALL_ANGLES_25);

  if ( abs(sysInfo.accSmooth[ROLL])<800 && abs(sysInfo.accSmooth[PITCH])<800) {
    f.SMALL_ANGLES_25 = 1;
  } else {
    f.SMALL_ANGLES_25 = 0;
  }
  ...
  //for (axis = 0; axis < 3; axis++) {
    //if ( (int16_t)(0.85*sysInfo.acc_1G*sysInfo.acc_1G/256) < (int16_t)(accMag>>8) && (int16_t)(accMag>>8) < (int16_t)(1.15*sysInfo.acc_1G*sysInfo.acc_1G/256) )
     //EstG.A32[axis] += (int32_t)(sysInfo.accSmooth[axis] - EstG.A16[2*axis+1])<<(16-GYR_CMPF_FACTOR);
    //accZ_tmp += sysInfo.accSmooth[axis] * EstG.A16[2*axis+1];
    //#if MAG
